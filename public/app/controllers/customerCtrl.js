...
.controller('customerCtrl', function($http, $location, $timeout, $scope, Customer, GLAccount, Config, Office) {
    ...
    function getOffices() {
        Office.getOffices().then(function(data) {
            ...
            if (data.data.success) {
                // Check which permissions the logged in user has
                if (data.data.permission === 'admin' || data.data.permission === 'moderator') {
                    app.offices = data.data.offices; // Assign customers from database to variable
                    app.loading = false; // Stop loading icon
                    app.accessDenied = false; // Show table
...
.controller('editCustomerCtrl', function($scope, $routeParams, User, Customer, $timeout, $location, GLAccount, Config, Office) {
    ...
    function getCustomers() {
        Customer.getCustomer($routeParams.id).then(function(data) {
            ...
            if (data.data.success) {
                ...
                $scope.newPrimaryInfo.offices = data.data.customer.offices;
    ...
    app.updatePrimaryInfo = function(newPrimaryInfo, valid) {
        ...
        if (valid) {
            ...
            customerObject.offices = newPrimaryInfo.offices;
